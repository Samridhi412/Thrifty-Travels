...
class Booking(webdriver.Chrome):
    def __init__(self, driver_path=r"C:\selenium_driver", teardown=False):
        ...
        # to instantiate inherited class
        super(Booking, self).__init__(options=options)
        self.implicitly_wait(15)
        self.maximize_window()  #clean
        # implicit_wait- allows to wait for sometime until element is available on website
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.teardown:
            self.quit()  #shut down chrome browser
    def land_first_page(self):
        self.get(const.BASE_URL)
    def change_currency(self, currency=None):
        print("entered")
        # cross_ele = self.find_element_by_class_name(
        #     "bb0b3e18ca bad25cd8dc d9b0185ac2 ba6d71e9d5 da7be6cac7"
        # )
        # cross_ele.click()
    ...
    def select_adults(self, count):
        ...
        while True:
            ...
            #if value of adults reaches 1, we should get out of while loop
            adults_value_element = self.find_element_by_id('group_adults')
            adults_value = adults_value_element.get_attributes('value') #should give back adults count
    ...
    def report_results(self):
        ...
        table.add_row(report.pull_deal_box_attributes())
        print(table)
